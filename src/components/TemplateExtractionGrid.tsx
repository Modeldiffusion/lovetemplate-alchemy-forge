...
export const TemplateExtractionGrid = () => {
  ...
  const handleExtractSingle = (templateId: string) => {
    navigate(`/extraction/${templateId}`);
  };

  const handleBulkExtract = () => {
    if (selectedTemplates.length > 0) {
      navigate(`/extraction/bulk?templates=${selectedTemplates.join(',')}`);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            Extract Selected ({selectedTemplates.length})
          </Button>
          <Button onClick={() => refetch()} variant="outline">
            <RefreshCw className="w-4 h-4 mr-2" />
            Refresh
